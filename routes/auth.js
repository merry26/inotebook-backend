...
const JWT_SECRET = "Thisissecretkeyforinotebooksite";

// ROUTE 1: Create a User using: POST "/api/auth/createuser".
router.post('/createUser', [
    body("name","Enter valid name").isLength({min:3}), 
    body("email","Enter valid email id").isEmail(), 
    body("password","Please enter password minimum 5 characters").isLength({min:5}), 
], async(req, res) => {
  let success = false;
    ...
    //checking user with same email exists
    try{
    ...
    user=await User.create({
      ...
      success=true;
      res.json({success, authtoken})//print user
    }
      catch(error){
       console.log(error.message)
       //print error of system
